Public Class Form1
    ...
    Private Sub BtnHapus_Click(sender As Object, e As EventArgs) Handles BtnHapus.Click
        If txtNIM.Text <> "" Then
            Dim confirmResult As DialogResult = MessageBox.Show("Apakah Anda yakin ingin menghapus data mahasiswa?", "Konfirmasi Hapus", MessageBoxButtons.YesNo)
            If confirmResult = DialogResult.Yes Then
                oMahasiswa.nim = txtNIM.Text
                oMahasiswa.DeleteState = False
                oMahasiswa.Hapus()

                If oMahasiswa.DeleteState = True Then
                    MessageBox.Show("Data mahasiswa berhasil dihapus.")
                    ClearEntry()
                    Reload()
                Else
                    MessageBox.Show("Gagal menghapus data mahasiswa.")
                End If
            End If
        Else
